...
export default function GameReviewModal({ username, onClose }) {
  ...
  const { data, isLoading } = trpc.gameReviewRouter.getGameReview.useQuery({ username, year, month, page }, { enabled: !!username })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
      <div className="bg-gray-900 text-white w-full max-w-2xl p-6 rounded-xl shadow-lg border border-gray-700 overflow-hidden">
        ...
        {data && (
          <div>
            <p className="text-sm text-gray-400 mb-4">Page {page} of {data.totalPages}</p>

            <div className="max-h-96 overflow-auto text-sm border border-gray-700 bg-gray-800 rounded-md p-4 space-y-3">
              {data.games.map((game, i) => (
               <div key={i} className="p-2 bg-gray-700 rounded-md border border-gray-600 hover:bg-gray-600 flex flex-col gap-3">
            ...
            <div className="flex justify-between items-center mt-4">
              <button onClick={handleNext} className="px-4 py-1 bg-gray-700 text-white rounded hover:bg-gray-600 disabled:opacity-50">
                <ArrowLeft />
              </button>
              <button onClick={handlePrev} disabled={(month === currentMonth && year === currentYear && page === 1)}
